set -u
set -e
debootstrap --components=main,universe,multiverse --include=vim,build-essential,git,redis-server,lua5.1,postgresql,libpq-dev,python-dev,python3-dev,memcached,mongodb,libperl-dev,ruby,wget,ruby-dev,language-pack-en,libcurl4-openssl-dev,mysql-server,libyajl-dev,beanstalkd,ssh,rsync,libluajit-5.1-dev,curl,ipython,liblocal-lib-perl,python-virtualenv,python-pip,libpcre3-dev,libjansson-dev,quota,ruby1.9.1-dev,rubygems,ruby-bundler,libjpeg-dev,libpng12-dev,nullmailer,nano,htop,emacs,libxml2-dev,libxslt-dev,mercurial,language-pack-it,language-pack-de,language-pack-es,language-pack-pt,language-pack-pl,tmux,libreadline-dev,strace,libsasl2-dev,screen,apache2-utils,libtiff-dev precise /distros/precise
